...
| **R2** | Contextual explanations rather than generic fitness tracking | Phases 2–4 | **Phase 2 Delivered** (Rule Engine) | Anomaly explanations cite exact baseline deviations and environmental factors rather than generic workout advice. |
...
| **R7** | Standardized 0–100 risk bands and transparent "Prototype AI Risk Score" disclosure | Phases 2–3 | **Phase 2 Delivered** (Rule Engine) | Exact bands: 0–30 Low (Green), 31–60 Moderate (Amber), 61–80 High (Orange), 81–100 Critical (Red). Explicit label "Prototype AI Risk Score — Rule-based demonstration". |
| **R8** | Five analysis stages, input/context explanation, replaceable engine interface | Phases 2–3 | **Phase 2 Delivered** (Contract & Engine) | Stages: Sensor Receipt → Baseline Comparison → Environmental Context → Pattern Analysis → Risk Assessment. Real current inputs fed to transparent rule engine. |
| **R9** | Personal baseline (HR, SpO₂, activity, sleep) and live percentage / percentage-point comparison | Phases 2–3 | **Phase 2 Delivered** (Baseline Manager) | Medians computed from normal history; HR shows relative % deviation (`(current - baseline)/baseline * 100`); SpO₂ shows absolute percentage-point difference. User edit takes precedence until recalculation. |
| **R10** | Five anomaly categories: cardiovascular, respiratory, heat stress, fatigue, and fall/motion | Phases 2–5 | **Phase 2 Delivered** (Rule Engine) | Category scores start at 18, add factor contributions, clamp to 0–100; overall score is maximum category score. Null confidence truthfully disclosed. |
...
| **R12** | Pollution scenario visibly links AQI and personal SpO₂ baseline drop | Phases 2–4 | **Phase 2 Delivered** (Engine & Fixtures) | AQI 185 + SpO₂ drops from 98% to 94% (4 percentage points); respiratory score reaches 68 (High); transparent factor contribution displayed. |
| **R13** | Heat scenario changes physiological and environmental values and produces explained high risk | Phases 2–4 | **Phase 2 Delivered** (Engine & Fixtures) | Ambient 40°C, humidity 78%, HR 108 (+44%), body temp 37.7°C, 45 min exposure → Heat score reaches 78 (High); 90 min exposure test reaches 93 (Critical). |
...
| **R15** | Demo offline switch plus actual production PWA offline reload and local operation | Phases 2, 7 | **Phase 2 Delivered** (Demo Flag) | PWA service worker precaches all assets/chunks; app reloads under real browser network-offline; local monitoring, analysis, alerts, and SOS preparation continue uninterrupted. |
...
| **R17** | SOS confirmation dialog, consent-filtered payload preview, simulated local preparation | Phases 5–6 | **Phase 2 Delivered** (Emergency Manager) | Confirmation with Cancel / Send SOS (demo); prepares local record with contact, timestamp, vitals, location (if permitted); status: "Prepared — demonstration only; nothing sent". |
| **R18** | Integrated Health Belt view, 10 component status rows, Connect Device flow and adapter boundary | Phases 2, 5 | **Phase 2 Scaffolding** (Device Manager) | Restrained SVG belt illustration; MPU6050, HR, SpO₂, temp, ESP32, buzzer, GPS, battery, relay, solenoid, CO₂ cartridge, airbag telemetry; Connect/Disconnect simulation. |
...
| **R20** | Dynamic Alerts workspace with deduplication, episode lifecycle, input snapshots, acknowledgement | Phase 4 | **Phase 2 Delivered** (Alert Manager) | Trigger after 2 consecutive qualifying samples; resolve after 3 normal samples; deduplicated by category episode; input snapshots frozen at alert time; works offline. |
...
| **R24** | Changing realistic demo inputs, including scripted motion followed by inactivity | Phases 2, 5 | **Phase 2 Delivered** (Simulator) | Root 2-second interval simulator with smooth ~6s transitions; Possible Fall scenario scripts high acceleration spike followed by zero motion. |
